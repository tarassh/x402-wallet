    ...
    fi
    {
      ...
      awk -v repo="$REPO_DIR" '
        /^The user invoked / && !injected {
          print "**Working directory:** Always run shell commands from `" repo "`. If your current directory is something else, prefix with `cd " repo " && ` before each `bun run cli` invocation."
          print ""
          injected = 1
        }
        { print }
